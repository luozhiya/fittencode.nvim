...
function View:_initialize(options)
    ...
    if self.col_delta ~= 0 then
        self.replaced_text = Fn.get_lines(self.buf, Range.of(self.origin_pos, self.last_insert_pos))
        ...
        for i, line_state in ipairs(lines) do
            for j, lstate in ipairs(line_state) do
                ...
                if lstate.type == 'commit' or lstate.type == 'placeholder' then
                    ...
                    self.last_insert_pos = Fn.calculate_cursor_position_after_insertion(self.last_insert_pos, packed)
