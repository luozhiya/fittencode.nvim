...
local function decimal_to_utf8(decimal)
...
-- * Unicode 序列格式为 \uXXXX\uXXXX，其中 X 为 0-9A-Fa-f 的 16 进制数字
-- * 例如 \u4E2D\u6587 表示中文
function M.unicode_sequence_to_utf8(seq)
    local v = vim.split(seq, '\\u', { trimempty = true })
    local s = ''
    for i, code in ipairs(v) do
        local c = decimal_to_utf8(tonumber(code, 16))
        s = s .. c
    end
    return s
end
